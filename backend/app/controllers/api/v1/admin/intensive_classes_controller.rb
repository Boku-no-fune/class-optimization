class Api::V1::Admin::IntensiveClassesController < Api::V1::Admin::BaseController
  before_action :set_class, only: [:show, :update, :destroy]

  def index
    @classes = IntensiveClass.includes(:course, :classroom).order(:year, :intensive_name, :name)
  ...
  def create
    @class = IntensiveClass.new(class_params)
    if @class.save
  ...
  def set_class
    @class = IntensiveClass.find(params[:id])
  rescue ActiveRecord::RecordNotFound
    render_not_found("IntensiveClass")
  end

  def class_params
    params.require(:intensive_class).permit(
      :year, :intensive_name, :name, :grade, :course_id, :classroom_id,
